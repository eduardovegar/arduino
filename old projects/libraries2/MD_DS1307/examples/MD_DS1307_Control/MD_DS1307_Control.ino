...
void usage(void)
{
  ...
  Serial.println("wt yyyymmdd hhmmss dw\twrite the current date, time and day of week (1-7)");
  Serial.println("wr aa nn vv [vv...]\twrite RAM address hex aa with nn hex values vv");
  Serial.println("rt\tread the current time");
...
void loop()
{
   ...
   switch (toupper(c))
   {
     ...
     // Write functions
     case 'W':  // Display updates      
       c = ReadNext();
       switch (toupper(c))
       {
         case 'R': writeRAM();   break;
         case 'T': writeTime();  break;
         default: goto no_good;
       }
       break;
